...
object UserRequestStoreAuthMiddleware {
  ...
  val userRequestStoreAuthMiddleware : RequestHandlerMiddleware[Nothing, ApiKeyStore with UserRequestStore, Throwable, Any] =
    new RequestHandlerMiddleware.Simple[ApiKeyStore with UserRequestStore, Throwable] {
      override def apply[Env <: ApiKeyStore with UserRequestStore, Err >: Throwable](handler: Handler[Env, Err, Request, Response])(implicit trace: Trace): Handler[Env, Err, Request, Response] =
        Handler
          .fromFunctionZIO[Request] { request =>
            val maybeHeader = request.headers.header(Utils.apiKeyHeader).map(_.value.toString)
            ApiKeyStore.getApiKey(maybeHeader.getOrElse("")).flatMap {
              ...
              case Some(ApiKey(email, _, ApiKeyType.User, _)) =>
                UserRequestStore.saveUserRequest(UserRequest(email, Instant.now(), request.toString)).flatMap {
                  case true => // UserRequest was successfully saved
                    for {
                      numberOfRequests <- UserRequestStore.countUserRequests(email) // check how many requests user did
                      numberOfCredits <- ZIO.succeed(Utils.defaultNumberOfCredits) // check number of available credits for user by email
                    } yield {
